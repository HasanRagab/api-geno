...
export function normalizeSchema(schema: unknown): Schema {
  ...
  for (const field of arrayFields) {
    if (Array.isArray((schema as any)[field])) {
      ;(normalized as any)[field] = (schema as any)[field].map((sub: unknown) => normalizeSchema(sub))
    }
  }

  const booleanNumberStringFields = ['nullable', 'minLength', 'maxLength', 'pattern', 'minimum', 'maximum', 'exclusiveMinimum', 'exclusiveMaximum'] as const
...
export function toParameterList(value: unknown): OpenAPIParameter[] {
  ...
  return value
    ...
    .filter((param) => param.name && ['query', 'path', 'header', 'cookie', 'body'].includes(param.in))
}

export function createQuerySchemaReference(endpoints: Endpoint[], schemas: Record<string, Schema>): void {
...
export function selectRequestContentType(content: Record<string, { schema?: unknown }> | undefined): string | undefined {
  if (!content) return undefined
  const candidate = Object.keys(content).find((ct) => ['application/json', 'application/x-www-form-urlencoded', 'multipart/form-data'].includes(ct))
