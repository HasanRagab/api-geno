...
function getQueryParamsRef(
	operationId: string,
	queryParams: Parameter[],
): string | undefined {
	return queryParams.length > 0 ? `${operationId}QueryParams` : undefined;
}
...
function buildEndpoint(
	...
	queryParamsRef: string | undefined,
): Endpoint {
	return {
		path,
		method: method.toUpperCase() as Endpoint["method"],
		operationId: getOperationId(d, method, path),
...
function getSuccessResponseRef(d: RawObject): string | undefined {
	...
	return extractSchemaRef(responseSchema);
}
...
function parseV3(spec: unknown): OpenAPIModel {
	...
	for (const [path, methodsRaw] of Object.entries(paths)) {
		...
		for (const [method, detailsRaw] of Object.entries(methods)) {
			...
			const responseRef = getSuccessResponseRef(d);
			const queryParams = parameters.filter((p) => p.in === "query");
			const operationId = getOperationId(d, method, path);
			const queryParamsRef = getQueryParamsRef(operationId, queryParams);
...
function parseV2(spec: unknown): OpenAPIModel {
	...
	for (const [path, methodsRaw] of Object.entries(paths)) {
		...
		for (const [method, detailsRaw] of Object.entries(methods)) {
			...
			const d = isObject(detailsRaw) ? detailsRaw : {};
			const mergedParams = [...pathParams, ...toParameterList(d.parameters)];
...
export function parseOpenAPIContent(content: string): OpenAPIModel {
	...
	if (version.startsWith("3.")) return parseV3(spec);
	if (version.startsWith("2.")) return parseV2(spec);
...
export function parseOpenAPI(filePath: string): OpenAPIModel {
	return parseOpenAPIContent(fs.readFileSync(filePath, "utf-8"));
}
